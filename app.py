...
# ============================================================
# 6. ПРЕДСКАЗАНИЕ (ПРОСТАЯ МОДЕЛЬ)
# ============================================================
st.markdown("---")
st.header("Прогноз продаж в Японии")

st.markdown("""
**Как это работает:** Модель анализирует похожие игры и усредняет их продажи в Японии.
""")
...
if st.button("Рассчитать прогноз", type="primary"):
    # Простая модель: ищем похожие игры
    similar = df[
    ...
    if len(similar) > 0:
        ...
        # Корректировка на основе продаж
    ...
    if pred_genre == 'RPG':
        st.write("- RPG жанр: +50% к продажам в Японии")
    if pred_publisher == 'Nintendo':
        st.write("- Издатель Nintendo: +40% к продажам в Японии")
